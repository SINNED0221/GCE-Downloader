...
def download():
    ...
    loadingPop.iconbitmap(resourcePath('icon.ico'))
    ...
    def coreDownload():
        ...
        def monoDownload(url, number):
            ...
            try:
                ...
                #  ^doesnt work anymore, the heads are now 403 forbidden
                ...
                fails += 1
                print("An error occurred:", type(e).__name__, "-", e.args[0])
        ...
        completionPop.iconbitmap(resourcePath('icon.ico'))
...
def popUpCR(*args):  # credit page
    cRpop = Toplevel(root)
    cRpop.iconbitmap(resourcePath('icon.ico'))
...
def getAppdataDir():
    ...
    return os.path.join(appdataDir, 'GCE_download_history.txt')
...
# file list box
fileListBox = Listbox(fileListFrame, width=30, height=7, selectmode=EXTENDED)
# listbox scroll bar
