...
def updateYear(*args):  # to make new subjects display correct years
    ...
    # for the subjects that has stopped, the previous years are removed
...
def updateLatestYear():
    def coreLatestYear():
        ...
        if not count > 10:  # ignore if there is connection error
            ...
            downloadingLabel.place(relx=.5, rely=.5, anchor=CENTER)
            downloadingLabel.pack
            #closeButton = Button(loadingPop, text='OK', command=loadingPop.destroy(), width=30)
            #closeButton.place(relx=.5, rely=.5, anchor=CENTER)
            #closeButton.pack
        else:
